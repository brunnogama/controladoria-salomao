...
import { LayoutDashboard, FileText, Users, History, Settings, LogOut, UserCircle, Menu, X } from 'lucide-react';
...
const Sidebar = () => {
  ...
  return (
    <>
      {/* Botão para Mobile real (< 768px) */}
      <button onClick={() => setIsOpen(!isOpen)} className="md:hidden fixed top-4 left-4 z-[60] p-2 bg-[#0F2C4C] text-white rounded-lg shadow-lg">
        {isOpen ? <X size={24} /> : <Menu size={24} />}
      </button>

      {isOpen && <div className="fixed inset-0 bg-black/50 z-[50] md:hidden" onClick={() => setIsOpen(false)} />}

      {/* Sidebar aparece fixa em Laptops (md:translate-x-0) */}
      <aside className={`fixed left-0 top-0 h-screen bg-[#0F2C4C] text-white z-[55] shadow-xl transition-transform duration-300 w-64 ${isOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
