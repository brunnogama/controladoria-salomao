...
const Dashboard = () => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div>
        <h1 className="text-2xl lg:text-3xl font-black text-[#0F2C4C] tracking-tight">Painel de Controle</h1>
        <p className="text-gray-500 text-sm font-medium">Resumo das atividades da controladoria hoje.</p>
      </div>

      {/* MÉTRICAS: 1 col no celular, 2 em tablets pequenos, 4 em Laptops/Desktops */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        {stats.map((stat, index) => (
          <div key={index} className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4 hover:shadow-md transition-shadow">
            <div className={`${stat.color} p-3 rounded-xl text-white shadow-lg`}>
              {stat.icon}
            </div>
            <div>
              <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{stat.label}</p>
              <p className="text-xl lg:text-2xl font-bold text-[#0F2C4C]">{stat.value}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
          ...
          <div className="h-64 bg-gray-50 rounded-2xl flex items-end justify-around p-4 gap-2">
            {[45, 60, 40, 85, 55, 75, 90, 60, 45, 80, 50, 70].map((h, i) => (
              <div key={i} className="bg-[#0F2C4C] w-full max-w-[30px] rounded-t-lg transition-all hover:bg-yellow-500" style={{ height: `${h}%` }}></div>
            ))}
          </div>
        </div>
