...
const GED = () => {
  ...
  const agrupados = docsFiltrados.reduce((acc, doc) => {
    ...
    <div className="p-4 md:p-8 space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 font-medium">Acervo digital de contratos vinculados.</p>
        </div>
        <div className="relative w-full md:w-80">
          ...
          <input 
            ...
            className="w-full pl-12 pr-4 py-3 bg-white border rounded-2xl outline-none focus:ring-2 focus:ring-[#0F2C4C] shadow-sm text-sm"
      ...
      {Object.keys(agrupados).length === 0 ? (
        <div className="text-center py-20 bg-gray-50 rounded-3xl border-2 border-dashed border-gray-200 text-gray-400">
          Nenhum documento PDF encontrado no sistema.
        </div>
      ) : (
            ...
            <div key={cliente} className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
              ...
              <div className="divide-y divide-gray-50">
                {agrupados[cliente].map(doc => (
                  <div key={doc.id} className="p-5 flex flex-col md:flex-row md:items-center justify-between gap-4 hover:bg-gray-50 transition-colors">
                    <div className="flex items-center gap-4">
                      <div className="bg-red-50 p-3 rounded-xl text-red-600"><FileText size={24} /></div>
                      <div>
                        <p className="font-bold text-gray-800 text-sm">{doc.descricao_contrato || 'Contrato Sem Descrição'}</p>
                    ...
                    <a 
                      ...
                      target="_blank" 
                      rel="noreferrer" 
                      className="bg-[#0F2C4C] text-white px-5 py-2.5 rounded-xl text-[10px] font-black uppercase flex items-center gap-2 hover:bg-blue-900 transition-all shadow-lg shadow-blue-900/10"
                    >
